...
def _has_timing_pattern(text: str) -> bool:
    # Accept things like 119.0, 132.5, 119.0-132.5, 119.0–132.5, 119 to 132.5
    patterns = [
        r"\d+\.\d+",
        r"\d+\s*[-–]\s*\d+",
        r"\d+\.\d+\s*[-–]\s*\d+\.\d+",
...
def validate_reasoning_json(reasoning: Dict[str, Any], compact_case: Dict[str, Any], schema_path) -> Dict[str, Any]:
    ...
    # 4. integration summary must mention confidence impact when mismatch flags are present
    ...
    if has_mismatch_flag:
        ...
        if not any(word in integration_summary_lower for word in ["confidence", "limit", "lower", "reduce", "cautious"]):
            errors.append(
                "integration_assessment.summary must explicitly mention confidence limitation or reduction when cross_modal_mismatch is present"
            )
